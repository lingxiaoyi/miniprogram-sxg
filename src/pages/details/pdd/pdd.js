...
class Pdd extends Component {
  ...
  /* componentWillReceiveProps(nextProps) {
    ...
    if (typeof accid !== 'undefined' && thisAccid !== accid) {
      this.timer.push(
        ...
        setTimeout(() => {
          dispatch(pddActions.loadAsync(this.goodsId, accid));
          if (!this.shareAccid) {
            this.loadPromotionUrl(accid, 'Own'); // 获取转链
          }
        }, 1)
  ...
  async loadPromotionUrl(accid, opt) {
    ...
    await this.props.dispatch(promotionUrlActions.loadPddAsync(this.goodsId, customParameters));
  }
  ...
  async handleShare() {
    ...
    Log.click({ buttonfid: 'x_10133', goodsid: this.goodsId, goodssource: 'pingduoduo' });
    // 分享进入（不登录也可以分享）
  ...
  savePic() {
    Log.click({ buttonfid: 'x_10136', goodsid: this.goodsId, goodssource: 'pingduoduo' });
  ...
  preview(e) {
    e.stopPropagation();
    Log.click({ buttonfid: 'x_10137', goodsid: this.goodsId, goodssource: 'pingduoduo' });
    Taro.previewImage({
   ...
   */
  async handleCopyTxt() {
    Log.click({ buttonfid: 'x_10138', goodsid: this.goodsId, goodssource: 'pingduoduo' });
    if (!this.shareAccid) {
      // 自购进入（非分享进入）
  ...
  copy() {
    ...
    Log.click({ buttonfid: 'x_10139', goodsid: this.goodsId, goodssource: 'pingduoduo' });
  }
          ...
          <View className='intro'>
            ...
            <View className='info'>
              <View className='price'>
                ...
                <Text className='old'>原价￥{oldPrice}</Text>
              </View>
